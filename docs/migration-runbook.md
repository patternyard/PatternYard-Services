...
4. Run each implemented collection with `migrate <collection> --dry-run`. Account-dependent collections must run in this order: `users`, `accountCustomization`, `loggedIPs`, `followers`, `oauthIDs`, `blocking`, `projects`, `projectStats`, then `messages`, `userFeed`, and `reports`. Project metadata intentionally runs after users; its remix links are restored in a second pass after all project rows exist. Communication collections run last because their rows reference accounts and, where applicable, projects.
